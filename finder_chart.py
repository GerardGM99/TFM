# -*- coding: utf-8 -*-
"""
Nadia's "finder_chart.py" form her github (https://github.com/nblago/utils/tree/master)

Added "draw image". Same structure that "finder_chart" but only draws the image
and plots/saves it.
"""
...
def query_sky_mapper_catalogue(ra, dec, radius_deg, minmag=8, maxmag=18.5):
    '''
...
def query_ps1_catalogue(ra, dec, radius_deg, minmag=8, maxmag=18.5):
    '''
...
def get_fits_image(ra, dec, rad, debug=True):
    ...
    try:
        image = fits.open("/tmp/tmp.fits", ignore_missing_end=True)
        ...
        try:
            fits.open("/tmp/tmp.fits", ignore_missing_end=True)
        except IOError:
